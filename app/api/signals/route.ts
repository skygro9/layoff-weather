...
// Common company name to ticker fallback map
...
export async function GET(req: NextRequest) {
  ...
  // ── 1. TICKER LOOKUP ─────────────────────────────────────────────────────────
  // First check our hardcoded map, then try Finnhub search
  ...
  if (!ticker) {
    try {
      ...
      const usStock = results.find(
        (r: any) => r.type === "Common Stock" && r.displaySymbol.indexOf(".") === -1
      );
      ticker = usStock ? usStock.symbol : (results[0]?.symbol || "");
    } catch (e) {
      console.error("Finnhub ticker lookup failed", e);
    }
  }
  ...
  const LAYOFF_TERMS = [
    "reduction in force", "workforce reduction", "headcount reduction",
    "job elimination", "restructuring plan", "severance",
    "workforce realignment", "reorganization"
  ];
  const EXEC_TERMS = [
    "departure", "resignation", "appointed", "chief executive",
  ...
  const RISK_KEYWORDS = [
    ...
    "cost cutting", "workforce reduction", "headcount", "right-sizing",
    "streamlining", "hiring freeze", "downsizing", "redundancies", "reorg"
  ];
    ...
    for (const article of articles) {
      ...
      for (const kw of RISK_KEYWORDS) {
        if (text.includes(kw)) {
          ...
          if (kw.includes("contractor")) contractorCuts = true;
        }
      }
  ...
  const layoffSeverity = recentLayoffs
    ? Math.min(85, 50 + allKeywords.length * 6)
    : Math.min(30, allKeywords.length * 5);
  if (stockTrend === "down" && newsSentiment === "negative" && hiringTrend === "flat") {
    hiringTrend = "decreasing";
  }

  return NextResponse.json({
    stockTrend, stockChangePercent, ticker,
    newsSentiment, keywords: allKeywords,
